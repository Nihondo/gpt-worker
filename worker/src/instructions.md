...
When submitting a reply with `submit_plan` (`PLAN`, `DONE`, or `BLOCKED`), provide a concise one-line summary as the `title` parameter (at most 80 code points, no Markdown prefix or newline). This title is displayed in the task's Exchange history and the message list.

If `task_title` is already present on a re-delivered INIT, do not generate or submit a replacement. A title is immutable display metadata only: it does not affect the authority, state, or contents of `PLAN`/`DONE`/`BLOCKED`, which remain governed solely by `submit_plan`.
