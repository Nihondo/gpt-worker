...
できること・できないこと:
- **できること**: キューに残っているメッセージ・acked 済みメッセージ・タスク履歴の閲覧（選択タスクでは `INIT` / `PLAN` / `EXECUTED` / 終端イベントを時系列で表示する本文なしの交換履歴を含む。本文は Messages の詳細ビューで確認できます）、`to_local` メッセージの ack、メッセージまたはタスク単位の discard、guidance とメッセージ本文サイズ上限の編集、新規タスクの投入（`gpt-worker task` と同様にローカルブリッジへ Chrome での表示・送信を best-effort で通知しますが、この通知が失敗してもタスク自体のキュー投入は成功したままです）——これらはワークスペース単位・hub 横断のどちらのダッシュボードからも行えます。
- **できないこと（現時点ではスコープ外）**: ChatGPT Project URL（`chat-url`）の編集・表示（この設定はローカルブリッジを実行しているマシン側にのみ存在し、Worker 側には保存されません）、WebSocket/SSE によるプッシュ型のライブ更新（画面の手動リロードなしでの更新自体は約10秒間隔のポーリングで既に行われており、ここでできないのはサーバー側からのプッシュ配信のみです）。
