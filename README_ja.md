...
- **Web ダッシュボード**：自分自身の Worker が、ワークスペースごとのブラウザダッシュボードも配信します。キュー状態・タスク履歴の閲覧、メッセージの ack/discard、guidance/limits の編集、CLI を使わない新規タスクの投入ができます。[Web ダッシュボード](#web-ダッシュボード)を参照してください。
...
### Web ダッシュボード
Cloudflare Worker は、一度に1ワークスペース分のブラウザダッシュボードも配信します。キュー状態・タスク履歴をカンバン風に可視化し、メッセージの ack/discard、guidance/limits の編集、CLI を使わずブラウザから新規タスクを投入することもできます。
...
`https://<your-worker>.workers.dev/dashboard/<workspace_id>` を開き、上記と同じ owner token（そのワークスペースの `gpt_token`）でログインします。**`-w` を付けない `gpt-worker url` が表示する共有 hub token ではログインできません**（拒否されます）。ログインすると、その owner token は 24 時間有効なブラウザセッション（`HttpOnly`/`Secure` Cookie）と交換されます——以降のリクエストで owner token 自体が再送されることはありません。

できること・できないこと:
- **できること**: キューに残っているメッセージ・acked 済みメッセージ・タスク履歴の閲覧、`to_local` メッセージの ack、メッセージまたはタスク単位の discard、guidance とメッセージ本文サイズ上限の編集、新規タスクの投入（`gpt-worker task` と同様にローカルブリッジへ Chrome での表示・送信を best-effort で通知しますが、この通知が失敗してもタスク自体のキュー投入は成功したままです）。
- **できないこと（現時点ではスコープ外）**: ChatGPT Project URL（`chat-url`）の編集・表示（この設定はローカルブリッジを実行しているマシン側にのみ存在し、Worker 側には保存されません）、共有 hub token でのログイン、複数ワークスペースの横断表示、WebSocket/SSE によるプッシュ型のライブ更新（画面の手動リロードなしでの更新自体は約10秒間隔のポーリングで既に行われており、ここでできないのはサーバー側からのプッシュ配信のみです）。
- 他の Worker 上のデータと同じ[データの保持期間](#データの保持期間)に従います。acked メッセージは7日後、完了（done/blocked）したタスクの履歴は30日後に消えます——ダッシュボードが CLI/ChatGPT 側より長く何かを保持することはありません。

そのワークスペースの owner token をローテーションした場合（`gpt-worker rotate --gpt -w .`）、またはワークスペースを削除した場合（`gpt-worker remove -w . --yes`）、ダッシュボードのセッションは即座に無効化されます——[データの保持期間](#データの保持期間)を参照してください。
...
- **[Web ダッシュボード](#web-ダッシュボード)のセッション**は24時間で失効し、自動的に掃除されます。ワークスペースの owner token をローテーションした場合や、ワークスペースを削除した場合も即座に無効化されます。
