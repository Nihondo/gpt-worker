...
- **Mac & Chrome による自動連携**: タスク送信時やレビュー時、Chrome の ChatGPT 画面を自動で開きます。そのワークスペースのタブを再利用できる場合は、メッセージの送信までバックグラウンド（フォーカスを奪わず）で自動化できます（`--auto-enter`）。
...
- `--auto-enter` オプションを指定すると、Chrome でタブを開いた後にメッセージを自動送信します。各ワークスペースには、実効 Project URL（個別 override、または共有デフォルト）に紐付く再利用可能な Chrome タブがあります。初回メッセージで ChatGPT の会話が作られた後は、以後の `task` と `report` は**同じ会話**へ継続メッセージを追加します。再利用タブを Project のランディングページへ戻したり、別の会話を作成したりしません。**そのタブを再利用できる場合、準備と送信は完全にバックグラウンドで行われます**（キーボードフォーカスを奪わず、他のウィンドウでの作業を妨げません）。一方、そのワークスペースで初回実行する場合（またはタブが閉じられた後の初回実行時）は新しい Chrome ウィンドウが作成されますが、これは他のアプリ同様、新規ウィンドウとして前面に表示されます — バックグラウンドのまま動作するのはタブを再利用できる場合のみです。タブを閉じた場合や Chrome を再起動した場合も、該当ワークスペースのタブだけを安全に作り直します。別 Project と通常の ChatGPT 会話のタブは再利用しません。`--auto-enter`、`--no-auto-enter`、`--enter-delay` は引き続きマシン全体の設定です。
- 既存会話を再利用するには、Chrome の **表示 → 開発 → Apple Eventsからのjavascriptを許可** を有効にして Chrome を再起動する必要があります（初回のみの設定、デフォルトは無効）。gpt-worker はこの設定を使って継続メッセージをその会話のコンポーザーへ入力し、`--auto-enter` では続けて送信します。キー入力送信によるフォールバックはありません。この設定を使えない場合、またはコンポーザーに未送信の下書きがある場合、既存会話には変更を加えず、必要な操作を表示します。
...
```
`chat-url` が設定されていれば、自動的に Chrome で対象 Project が開き、メッセージが準備（または送信）されます。ワークスペースの会話タブが作られた後は、以後の継続メッセージは同じ会話へ追加されます。
