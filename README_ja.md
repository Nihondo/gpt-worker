...
- **結果**: 文字のバッジで表示します（色だけには頼りません）。**OK**、**Gate**（ブロック：アクティブなタスクが無い、または読み取り窓が閉じている——「アクセス制限」を参照）、**Deny**（拒否：機密ファイル・Git 無視ファイル・ワークスペース外のファイル）、**NG**（エラー）、**Mix**（結果が混在するバッチ。開くと各呼び出しを確認できます）。バッジは収まるよう短くしてあり、詳細欄では「Blocked」「Denied」と正式な語で表示します。
