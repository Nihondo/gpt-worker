...
できること・できないこと:
- **できること**: キューに残っているメッセージ・acked 済みメッセージ・タスク履歴の閲覧、`to_local` メッセージの ack、メッセージまたはタスク単位の discard、guidance とメッセージ本文サイズ上限の編集、新規タスクの投入（`gpt-worker task` と同様にローカルブリッジへ Chrome での表示・送信を best-effort で通知しますが、この通知が失敗してもタスク自体のキュー投入は成功したままです）——これらはワークスペース単位・hub 横断のどちらのダッシュボードからも行えます。
