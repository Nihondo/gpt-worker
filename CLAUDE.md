...
Local files (`bridge/state.mjs`) hold only secrets/process bookkeeping — never a second copy of task/protocol state. The machine-wide `worker.json` may also hold the non-protocol `chromeTabsByWorkspace` UI mapping so one shared ChatGPT Project can keep a distinct Chrome tab per workspace; mutate that map through `updateWorkerConfigAtomic()` so simultaneous CLIs cannot overwrite another workspace's association. `state.json`/`guidance.md` in the local state dir are legacy migration inputs only (see `localMigrateLegacyState` in `worker/src/index.js`); do not add new local state that duplicates what the DO already owns.
